...
export const CharacterDetailPage = () => {
  ...
  const isFavorite = character
    ? favorites.some((item) => item.includes(character.id))
    : false;
    ...
    <div className="ml-[24px] mr-[24px] sm:ml-[100px] ">
      ...
      <div>
        <div className="grid grid-cols-1  gap-4 mt-4">
          ...
          <div>
            <div className="font-bold text-gray-900">Gender</div>
            <div className="font-medium text-gray-600">{character?.gender}</div>
          </div>
        </div>
      </div>
      {/* Renderizo solo si character existe  así me aseguro de no pasar nunca undefined. */}
      {character && <CommentBox characterId={character.id} />}
    </div>
  );
};
